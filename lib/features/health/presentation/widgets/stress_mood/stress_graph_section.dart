...
class StressGraphSection extends StatelessWidget {
  const StressGraphSection({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<StressMoodTrackingCubit, StressMoodTrackingState>(
      builder: (context, state) {
        ...
        final Map<String, List<int>> dailyStress = {};

        for (var record in records) {
          final key = DateFormat('yyyy-MM-dd').format(record.measurementTime);
          dailyStress.update(key, (value) => [...value, record.stress], ifAbsent: () => [record.stress]);
        }
        ...
        for (int i = 0; i < displayDays.length; i++) {
          final values = dailyStress[displayDays[i]]!;
          final avgStress = values.reduce((a, b) => a + b) / values.length;
          minY = min(minY, avgStress);
          maxY = max(maxY, avgStress);
        ...
        return Column(
          ...
          children: [
            Text(context.strings.stressGraph, style: context.textTheme.titleLarge),
            const Gap(8),
            AspectRatio(
              aspectRatio: 1.5,
              child: Padding(
