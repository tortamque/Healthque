...
class TemperatureRemindersPage extends StatelessWidget {
  const TemperatureRemindersPage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(context.strings.temperatureReminders),
      ),
      body: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          children: [
            const TemperatureReminderCard(),
            const Gap(16),
            Expanded(
              child: BlocBuilder<RemindersCubit, RemindersState>(
                builder: (context, state) {
                  ...
                  final notifications = state.temperatureNotifications;
                  if (notifications.isEmpty) {
                    return Center(
                      child: Text(context.strings.noNotifications),
                    );
                  }
